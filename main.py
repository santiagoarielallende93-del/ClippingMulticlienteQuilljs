...
# ====================================================================
APP_VERSION = "1.10"
URL_VERSION_GITHUB = "https://raw.githubusercontent.com/santiagoarielallende93-del/actualizaciones/refs/heads/main/version.txt"
...
def construir_bloque_texto(resumen_meta, oracion, titulo, palabras_clave="", sec_id="", resumen_rss=""):
    ...
    if sec_id in secciones_destacadas:
        if resumen_meta_limpio:
            html += f"<p style='font-size: 13px; font-family: Tahoma, sans-serif; color: #000000; margin-top: 0; margin-bottom: 6px;'>{resumen_meta_limpio}</p>"
        ...
        if oracion_final:
            ...
            if not resumen_meta_limpio or texto_oracion_limpio not in resumen_meta_limpio:
                html += f"<p style='font-size: 13px; font-family: Tahoma, sans-serif; color: #000000; margin-top: 0; margin-bottom: 6px;'>{oracion_final}</p>"
        else:
            if not resumen_meta_limpio:
                html += f"<p style='font-size: 13px; font-family: Tahoma, sans-serif; color: #888888; font-style: italic; margin-top: 0; margin-bottom: 6px;'>[Mención no detectada automáticamente en el texto visible]</p>"
    else:
        if resumen_meta_limpio:
            html += f"<p style='font-size: 13px; font-family: Tahoma, sans-serif; color: #000000; margin-top: 0; margin-bottom: 6px;'>{resumen_meta_limpio}</p>"
        else:
            html += "<p style='font-size: 13px; font-family: Tahoma, sans-serif; color: #888888; font-style: italic; margin-top: 0; margin-bottom: 6px;'>Sin resumen disponible.</p>"
...
def procesar_seccion(context, sec_id, nombre_seccion, lista_rss, links_manuales, notas_graficas_sec, palabras_clave, color_tema, limite_notas, logger, cliente_nombre, df_medios, solo_manuales=False):
    ...
    for noti in noticias_finales:
        ...
        tipo_html = f" <strong style='color: {color_tema};'>({noti['tipo_medio']})</strong> " if noti['tipo_medio'] != "Gráfica" else " "
        
        if sec_id == 'booking_tema_1':
            info_metricas = f" <strong style='color: {color_tema};'>Ad. Value: $ {noti['ad_value']}</strong> -"
        elif sec_id in IDS_SINTESIS:
            info_metricas = f" <span style='color: {color_tema};'>(Alcance: {noti['alcance']} Tier: {noti['tier']})</span> <strong style='color: {color_tema};'>Ad. Value: $ {noti['ad_value']}</strong> -"
        else:
            info_metricas = " -"
            
        html_indiv = f'''<p style="font-size: 14px; font-family: Tahoma, sans-serif; line-height: 1.5; margin-top: 0; margin-bottom: 4px;"><strong style="color: {color_tema};">{noti['medio']}</strong>{tipo_html}<strong style="color: {color_tema};">{noti['fecha']}</strong>{info_metricas} <a href="{noti['link']}" target="_blank" style="color: {color_tema}; text-decoration: none; font-size: 14px; font-weight: normal;">{noti['titulo']}</a></p>{bloque_texto}'''
...
def orquestador_principal(links_manuales, notas_graficas, configuracion_cliente, cliente_nombre, logger, timeframe_google, solo_manuales=False, solo_banners=False):
    ...
    # MODO DIOS: PRUEBA RÁPIDA (SOLO BANNERS)
    if solo_banners:
...
# ====================================================================
# GENERADOR HTML QUILL.JS CON FONDO GRÁFICO SEMI-TRANSPARENTE
# ====================================================================
...
<head>
    ...
    <style>
        ...
        /* FONDO CON PATRÓN GRÁFICO SEMI-TRANSPARENTE CON EL COLOR INSTITUCIONAL */
        body { 
        ...
        .seccion-header { background: var(--tema_color); color: white; padding: 12px 20px; font-weight: bold; display: flex; justify-content: space-between; }
        ...
        .ql-container.ql-snow { border: none !important; font-family: 'Tahoma', sans-serif !important; font-size: 14px !important; }
        .ql-editor { font-family: 'Tahoma', sans-serif !important; font-size: 14px !important; padding: 4px 0 !important; line-height: 1.5 !important; }
        .ql-editor p { font-family: 'Tahoma', sans-serif !important; font-size: 14px !important; line-height: 1.5 !important; margin: 0 0 6px 0 !important; color: #000000 !important; }
    ...
    <script>
        ...
        async function regenerarResumenIA(secIdx, btnElement) {
            ...
            try {
                ...
                if (qSin) {
                    qSin.root.innerHTML = `<p>${resultado}</p>`;
                } else {
                    render();
                }
            } catch (error) {
        ...
        function render() {
            ...
            if (secsSintesis.length > 0) {
                ...
                sinDiv.style.borderLeft = '4px solid var(--tema_color)';
                sinDiv.style.background = '#f8fafc';

                const sHeader = document.createElement('div');
                sHeader.className = 'seccion-header';
                sHeader.style.background = 'transparent';
                sHeader.style.color = 'var(--tema_color)';
                sHeader.innerHTML = '<span>✨ SÍNTESIS DEL DÍA · RESUMEN IA</span>';
                ...
                secsSintesis.forEach((sec) => {
                    ...
                    btnRegenerar.style.padding = '4px 10px';
                    ...
                    let contenidoInic = sec.resumen_ia ? `<p>${sec.resumen_ia}</p>` : `<p><em>Resumen IA vacío. Tocá el botón para generar la redacción.</em></p>`;
            ...
            estado.forEach((sec, secIdx) => {
                ...
                header.innerHTML = `<span>${sec.nombre}</span><span>${sec.notas.length} notas</span>`;
                secDiv.appendChild(header);
                
                if(sec.notas.length === 0) {
                     secDiv.innerHTML += `<div style="padding:20px; color: __COLOR_CLIENTE__; font-weight: bold; font-family: Tahoma, sans-serif; font-size: 14px; text-align: left;">No se produjeron menciones</div>`;
                } else {
            ...
            if (aTag) {
                ...
                if (nuevaUrl !== null && nuevaUrl.trim() !== '') {
                    ...
                    if (textHtml.includes('-')) {
                        ...
                        pTag.innerHTML = `${header}- <a href="${nuevaUrl.trim()}" target="_blank" style="color: __COLOR_CLIENTE__; text-decoration: none; font-size: 14px; font-weight: normal;">${cleanRest}</a>`;
                    } else {
                        let cleanText = pTag.innerText.trim() || 'Ver Nota';
                        pTag.innerHTML = `<a href="${nuevaUrl.trim()}" target="_blank" style="color: __COLOR_CLIENTE__; text-decoration: none; font-size: 14px; font-weight: normal;">${cleanText}</a>`;
                    }
        ...
        function generarHtmlFinal(){
            ...
            html += '<style>body, table, td, p, div, span, a { font-family: Tahoma, sans-serif !important; } p { font-family: Tahoma, sans-serif !important; font-size: 14px !important; line-height: 1.5 !important; margin: 0 0 6px 0 !important; color: #000000 !important; } a { text-decoration: none; }</style>';
            ...
            if(secsExc.length > 0){ 
                let html_sintesis = '<tr><td style="padding: 20px;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin: 8px 0 8px 0;"><tr><td style="background: #f0f4f4; border-left: 4px solid __COLOR_CLIENTE__; padding: 18px 22px; border-radius: 4px;"><p style="margin: 0 0 12px 0; font-family: Tahoma, sans-serif; font-size: 12px; font-weight: bold; color: __COLOR_CLIENTE__; letter-spacing: 0.5px; text-transform: uppercase;">✨ SÍNTESIS DEL DÍA · RESUMEN IA</p>';

                secsExc.forEach(sec => {
                    html_sintesis += '<p style="margin: 10px 0 4px 0; font-family: Tahoma, sans-serif; font-size: 14px; font-weight: bold; color: __COLOR_CLIENTE__;">' + sec.nombre + '</p>';

                    if (sec.resumen_ia && sec.resumen_ia.trim() !== '') {
                        html_sintesis += '<div style="margin: 0 0 10px 0; font-family: Tahoma, sans-serif; font-size: 13px; line-height: 1.5; color: #333333;">' + sec.resumen_ia + '</div>';
                    } else {
                        let items = [];
                        sec.notas.forEach(n => {
                            ...
                            items.push('<li style="margin-bottom: 4px;"><strong>' + medioReal + ':</strong> <a href="' + linkUrl + '" target="_blank" style="color: __COLOR_CLIENTE__; text-decoration: none;">' + tituloReal + '</a></li>');
                        });
                        html_sintesis += '<ul style="margin: 0 0 10px 0; padding-left: 18px; font-family: Tahoma, sans-serif; font-size: 13px; line-height: 1.4; color: #333333;">' + items.join('\n') + '</ul>';
                    }
                });
            ...
            estado.forEach((sec, index) => {
                ...
                if(sec.notas.length === 0) {
                    html += `<tr><td style="padding: 20px;"><p style="font-family: Tahoma, sans-serif; font-size: 14px; color: __COLOR_CLIENTE__; font-weight: bold; margin: 0;"><strong style="color: __COLOR_CLIENTE__;">No se produjeron menciones</strong></p></td></tr>`;
                } else {
                    sec.notas.forEach(n => { 
...
def index():
    ...
    # CHEQUEO AUTOMÁTICO DE VERSIÓN GITHUB AL INICIAR
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if v_git != APP_VERSION:
                    ui.notify(f'🚀 ¡Actualización disponible! Hay una nueva versión ({v_git}). Avisale a Santi para actualizar.', type='warning', position='top-right', timeout=8000)
        except: pass
    ...
    with ui.left_drawer(value=True).classes('bg-[#f8f9fa] border-r border-gray-200 p-6'):
        ...
        # MODO DIOS / HERRAMIENTAS ADMIN (SOLO VISIBLE PARA ADMIN)
        if app.storage.user.get('username') == 'admin':
            with ui.card().classes('w-full p-4 mb-6 border border-amber-300 bg-amber-50 shadow-sm rounded-xl'):
                ...
                async def ejecutar_solo_manuales():
                    await procesar(solo_manuales=True)
                    
                async def ejecutar_prueba_rapida():
                    await procesar(solo_banners=True)

                ui.button('📝 Reporte SOLO MANUALES', on_click=ejecutar_solo_manuales).props('dense outline text-color=amber-9').classes('w-full mb-2 font-bold')
                ui.button('⚡ Prueba Rápida (Solo Banners)', on_click=ejecutar_prueba_rapida).props('dense color=amber-8').classes('w-full font-bold text-white')
    ...
    def main_content():
        ...
        with ui.column().classes('w-full max-w-5xl mx-auto p-8'):
            ...
            timer_label = ui.label('⏱️ Tiempo transcurrido: 00:00').classes('font-bold text-gray-700 mb-2 hidden')
            log_container = ui.log(max_lines=30).classes('w-full h-48 bg-gray-900 text-green-400 p-4 rounded-lg font-mono text-sm hidden')
            
            async def procesar(solo_manuales=False, solo_banners=False):
                timer_label.classes(remove='hidden')
                log_container.classes(remove='hidden')
                log_container.push("🚀 Iniciando motor de procesamiento...")
                ...
                def update_chrono():
                    ...
                    timer_label.set_text(f'⏱️ Tiempo transcurrido: {mins:02d}:{secs:02d}')
                ...
                def flush_logs():
                    while not log_queue.empty():
                        log_container.push(log_queue.get())
                ...
                try:
                    ...
                    log_container.push("✅ PROCESO TERMINADO. Generando editor...")
                    html_resultado = generar_html_editor(config["banner_principal_url"], data_editor, config["color_primario"], state.cliente)
                    
                    ui.download(html_resultado.encode('utf-8'), f'Editor_{state.cliente}.html')
                    ui.notify('🎉 Reporte procesado y descargado exitosamente', color='positive', position='top')
                except Exception as e:
                    flush_logs()
                    ui_timer.deactivate()
                    ui_chrono.deactivate()
                    log_container.push(f"❌ Error durante el proceso: {str(e)}")
                    ui.notify('❌ Error al generar. Mirá el log de pantalla.', color='negative')

            ui.button('🚀 PROCESAR REPORTE', on_click=lambda: procesar()).classes('w-full py-4 text-lg font-bold shadow-lg rounded-xl mt-4').style(f'background-color: {color}; color: white;')
